...
public abstract class Subscriber extends RunnableSubPub {
    ...
    private final MessageBroker mbInstance;
    private Map<Class<? extends Message>, Callback> call_event;
    ...
    public Subscriber(String name) {
        ...
        this.mbInstance = MessageBrokerImpl.getInstance();
        this.call_event = new HashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        this.mbInstance.subscribeEvent(type, this);
        this.call_event.put(type, callback);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        this.mbInstance.subscribeBroadcast(type, this);
        this.call_event.put(type, callback);
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
        this.mbInstance.complete(e, result);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return this.mbInstance.sendEvent(e);
    }

    protected final void sendBroadcast(Broadcast b) {
        this.mbInstance.sendBroadcast(b);
    }
    ...
    public final void run() {
        int g = 0;
        this.mbInstance.register(this);
        initialize();

        while (!terminated) {
            try {
                Message message = this.mbInstance.awaitMessage(this);
                Callback call = call_event.get(message.getClass());
